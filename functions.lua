...
function Core.SavePlayer(xPlayer, cb)
	...
	exports["codem-inventory"]:SavePlayerInventory(xPlayer.source)
		MySQL.prepare(
...
function ESX.UseItem(source, item, ...)
    if ESX.Items[item.name] then
        ...
        if itemCallback then
            ...
            if not success then
                return result and print(result) or print(('[^3WARNING^7] An error occured when using item ^5"%s"^7! This was not caused by ESX.'):format(item.name))
            end
        end
    else
